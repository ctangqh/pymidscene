...
async def test_mcp_baidu():
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            print("✅ 发送初始化请求...")
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=init_payload, timeout=30) as event_source:
            ...
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=ctx_payload, timeout=30) as event_source:
                for event in event_source.iter_sse():
                    if event.event == "message":
            ...
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=page_payload, timeout=30) as event_source:
                for event in event_source.iter_sse():
                    if event.event == "message":
            ...
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=goto_payload, timeout=60) as event_source:
                for event in event_source.iter_sse():
                    if event.event == "message":
            ...
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=title_payload, timeout=30) as event_source:
                for event in event_source.iter_sse():
                    if event.event == "message":
            ...
            async with connect_sse(client, "POST", "http://172.10.0.25:8000/mcp", json=screenshot_payload, timeout=60) as event_source:
                for event in event_source.iter_sse():
                    if event.event == "message":
            ...
            print(f"   ✅ 功能测试：打开百度页面成功，标题正确，截图正常")
            print(f"   ✅ 页面标题：{title}")
            print(f"   ✅ 截图已保存：baidu_test_success.png")
